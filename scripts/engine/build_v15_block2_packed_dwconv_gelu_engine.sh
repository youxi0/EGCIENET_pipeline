...
# V15：四个 Block2 44x44x512 节点直接消费 token-major fc1 输出，
...
ONNX="${ONNX:-${PROJECT_ROOT}/models/egcienet_352_multiclass_qdq_v15_block2_packed_dwconv_gelu.onnx}"
...
if [ ! -s "${ONNX}" ]; then
    echo "[ERROR] V15 ONNX model not found: ${ONNX}" >&2
    echo "[INFO] generate it with scripts/onnx/fuse_block2_packed_dwconv_gelu.py" >&2
    exit 1
fi
if [ ! -s "${PLUGIN_SO}" ]; then
...
echo "[INFO] build V15 Block2 packed DWConv + GELU engine"
