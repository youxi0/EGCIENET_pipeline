...
# V21：Block3/4 的 attention Q projection 以及 Block2/3/4 的 KV
# projection 已插入显式 Q/DQ；Block1 KV 保持 FP16。显式量化不需要校准器。
BUILD_DIR="${BUILD_DIR:-${PROJECT_ROOT}/build}"
PLUGIN_BUILD_DIR="${PLUGIN_BUILD_DIR:-${PROJECT_ROOT}/build/packed_dwconv_plugin}"
ONNX="${ONNX:-${PROJECT_ROOT}/models/egcienet_352_multiclass_qdq_v21_q_block34_kv_block234.onnx}"
ENGINE="${ENGINE:-${PROJECT_ROOT}/models/egcienet_352_multiclass_qdq_v21_q_block34_kv_block234.engine}"
...
if [ ! -s "${ONNX}" ]; then
    echo "[ERROR] V21 ONNX model not found: ${ONNX}" >&2
    exit 1
fi
if [ ! -s "${PLUGIN_SO}" ]; then
...
echo "[INFO] build V21 attention Q/KV MatMul Q/DQ engine"
