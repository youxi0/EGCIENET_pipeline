...
# V22：Block1/3/4 的 attention Q projection 以及所有 Block 的 KV
# projection 已插入显式 Q/DQ；Block2 Q 保持 FP16。显式量化不需要校准器。
BUILD_DIR="${BUILD_DIR:-${PROJECT_ROOT}/build}"
PLUGIN_BUILD_DIR="${PLUGIN_BUILD_DIR:-${PROJECT_ROOT}/build/packed_dwconv_plugin}"
ONNX="${ONNX:-${PROJECT_ROOT}/models/egcienet_352_multiclass_qdq_v22_q_block134_kv_all.onnx}"
ENGINE="${ENGINE:-${PROJECT_ROOT}/models/egcienet_352_multiclass_qdq_v22_q_block134_kv_all.engine}"
...
if [ ! -s "${ONNX}" ]; then
    echo "[ERROR] V22 ONNX model not found: ${ONNX}" >&2
    exit 1
fi
if [ ! -s "${PLUGIN_SO}" ]; then
...
echo "[INFO] build V22 attention Q/KV MatMul Q/DQ engine"
