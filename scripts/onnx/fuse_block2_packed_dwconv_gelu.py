...
"""Replace the four Block2 layout/DWConv/GELU chains with packed plugins."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fuse Block2 token-major/NCHW layout conversion, 3x3 depthwise "
            "convolution and GELU into the packed DWConv TensorRT plugin."
        )
    )
    parser.add_argument("input", type=Path, help="source V14 ONNX model")
    parser.add_argument("output", type=Path, help="destination V15 ONNX model")
    return parser.parse_args()


def pack_half2_weights(weight: np.ndarray) -> list[int]:
    if weight.shape != (CHANNELS, 1, 3, 3):
        raise ValueError(f"unexpected Block2 DWConv weight shape: {weight.shape}")
...
def pack_half2_bias(bias: np.ndarray) -> list[int]:
    if bias.shape != (CHANNELS,):
        raise ValueError(f"unexpected Block2 DWConv bias shape: {bias.shape}")
...
def main() -> None:
    ...
    fused_nodes: list[gs.Node] = []

    for block_index in range(BLOCK_COUNT):
        prefix = f"/model/block2.{block_index}/mlp"
        ...
        if len(fc1.outputs) != 1 or len(conv.inputs) != 3:
            raise ValueError(f"unexpected Block2.{block_index} graph structure")
        if gelu_output.dtype != np.float16:
            raise ValueError(
                f"Block2.{block_index} plugin output must be FP16, got "
                f"{gelu_output.dtype}"
            )
        ...
        # Disconnect the exact-erf GELU producer. cleanup() removes the now-dead
        # Transpose/Reshape/Conv/shape/GELU subgraph and unused constants.
        gelu_output.inputs.clear()
        plugin = gs.Node(
            op=PLUGIN_OP,
            domain=PLUGIN_DOMAIN,
            name=f"{prefix}/Block2PackedDwconvGelu",
    ...
    block2_nodes = [
        node for node in custom_nodes if "Block2PackedDwconvGelu" in node.name
    ]
    if len(block2_nodes) != BLOCK_COUNT:
        raise RuntimeError(
            f"expected {BLOCK_COUNT} Block2 plugin nodes, got {len(block2_nodes)}"
        )

    print(f"[PASS] fused Block2 nodes: {len(block2_nodes)}")
