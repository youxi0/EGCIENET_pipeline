...
struct TensorRTInferConfig {
    ...
    // 动态 engine 使用该形状设置输入 tensor；静态 engine 据此校验输入形状。
    nvinfer1::Dims4 inputShape{1, 3, 352, 352};
};

// EGCINET TensorRT 执行封装。
// 本类只拥有 TensorRT runtime、engine、context 和 I/O tensor 元数据，
...
public:
    ...
    // 反序列化 engine、创建 execution context，并解析输入输出 tensor。
    bool load();
...
private:
    // 保存单个 I/O tensor 的运行时形状、类型和所需显存大小。
    struct TensorInfo {
        std::string name;
        nvinfer1::Dims dims{};
    ...
    bool inspectIOTensors();

    // 按 tensor 名称设置调用方显存并提交推理，不进行资源申请、同步或 D2H。
    bool enqueue(
        void* inputDevice,
        void* outputDevice,
        cudaStream_t stream
