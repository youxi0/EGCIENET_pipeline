...
constexpr int32_t kInputWidth = 22;
constexpr int32_t kChannels = 320;
constexpr int32_t kKernelWidth = 2;
constexpr int32_t kStride = 2;
constexpr int32_t kCtaTokens = 32;
constexpr int32_t kCtaChannels = 320;
constexpr int32_t kKStage = 128;
constexpr int32_t kChannelTilesPerWarp = 4;
constexpr int32_t kChannelsPerWarp = 32;
constexpr int32_t kPackedK = 1280;
constexpr int32_t kChannelWarps = 10;
constexpr int32_t kCtaThreads = 640;
...
constexpr int32_t kSharedMemoryBytes =
    2 * (kInputTileBytes + kWeightTileBytes); //双缓冲

static_assert(kSharedMemoryBytes == 88 * 1024);

// Block3 使用一个 CTA 覆盖全部 320 个输出通道。每个 warp 持有 4 个
// 相邻的 N=8 MMA tile，使同一个 32-token 输入窗口只需构造一次。
...
// 单条 PTX 指令完成 m16n8k32 的 INT8 Tensor Core MMA，并在 INT32 中累加。
...
// Block3 从 Q 分支复用 INT8 激活，可以直接异步搬入输入 tile。
...
__global__ void fusedInt8SpatialReductionKernel(
    ...
    // Block3 固定使用 88 KiB opt-in dynamic shared memory 保存双缓冲 tile。
    ...
    // 每个 warp 用四组 accumulator 复用同一份输入 fragment。
    int32_t accumulator[kChannelTilesPerWarp][4]{};
    copyStage(
        input,
        weight,
        inputTiles,
...
#pragma unroll
        for (int32_t fragment = 0;
            ...
            uint32_t weightFragments[kChannelTilesPerWarp][2];
#pragma unroll
            for (int32_t channelTile = 0;
                 channelTile < kChannelTilesPerWarp;
                 ++channelTile) {
                loadWeightFragment(
                    currentWeightTile,
...
// 启动固定形状的 Block3 kernel，并申请 88 KiB dynamic shared memory。
